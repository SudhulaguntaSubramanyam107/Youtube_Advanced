...
const Head = () =>{
    ...
    return (
       <div className={`flex items-center justify-between p-2 shadow-lg bg-gray-300 h-20 ${isDark ? "bg bg-gray-900" :"bg bg-gray-50"}`}>
                <div className="flex items-center gap-4">
                    <img
                    className="h-5 cursor-pointer"
                    src="https://upload.wikimedia.org/wikipedia/commons/thumb/b/b2/Hamburger_icon.svg/1280px-Hamburger_icon.svg.png"
                    alt="hamburger-icon" onClick={ ()=>toggleHandler()}/>
                    <img className="h-14 cursor-pointer" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRtzCvQhm0rpCNiwTurKc7d_LzYIFAYGjEeng&s" alt="Youtube-icon"/>
                </div>
               <div className="relative flex items-center">
                    ...
                    {showSuggestions && (
                        ...
                        <ul>
                            {suggestions.map((suggest)=> <li className= "hover:bg-gray-50 p-2 shadow-sm" key = {suggest}>🔍 {suggest}</li>)}
                        </ul>
                        </div>
                    )}
                    </div>
                 <div className="p-2 m-2">
                    <h1 className="cursor-pointer" onClick={()=>Themehandler()}>{isDark ? "🌕" :"🌙" }</h1>
                 </div>
                <div>
                    <img
